document.addEventListener('DOMContentLoaded', () => {
    ...
    async function startGame(game) {
        ...
        // **LA CORRECTION EST ICI** : On prépare les cartes pour chaque joueur AVANT de lancer la partie
        playerIds.forEach(id => {
            updatedPlayers[id].cards = deck.splice(0, 4).map(card => ({ ...card, revealed: false }));
        });

        // On prépare la pyramide
        ...
        // On envoie TOUT en une seule fois à Firebase
        await gameRef.update({
            status: "started",
            phase: "distribution",
            deck: deck, // Le deck restant après distribution
            pyramid: pyramid,
            players: updatedPlayers, // L'objet des joueurs mis à jour avec leurs cartes
            currentTurn: playerIds[0]
        });
        
        // La redirection se fera automatiquement grâce à l'écouteur principal
    }
});